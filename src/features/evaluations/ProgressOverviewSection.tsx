  ...
  return (
    <section id="progress-overview" className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-rose-50 sm:text-2xl">Progress insight</h2>
          <p className="text-sm text-rose-200/75">Quarter-to-date progression towards the season performance targets.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
        <RedSurface tone="muted" className="rounded-3xl p-6 text-rose-50">
          <header className="flex items-center justify-between">
            <h3 className="text-lg font-semibold text-rose-50">Performance trend</h3>
            <p className="text-xs uppercase tracking-[0.3em] text-rose-200/70">Season rating</p>
          </header>
              ...
              <div key={point.label} className="flex flex-col items-center gap-2">
                <RedSurface tone="glass" className="flex h-36 w-full items-end justify-center rounded-2xl p-2">
                  <div className="relative flex h-full w-6 flex-col justify-end">
                    ...
                    />
                    <div
                      className="absolute -top-2 left-1/2 h-3 w-3 -translate-x-1/2 rounded-full border-2 border-rose-200/70 bg-rose-950"
                      style={{ bottom: `calc(100% - ${point.target}%)` }}
                      aria-hidden
                    />
                  </div>
                </RedSurface>
                <p className="text-xs uppercase tracking-[0.3em] text-rose-200/70">{point.label}</p>
              </div>
            ))}
          </div>
        ...
        <RedSurface as="aside" tone="muted" className="flex flex-col gap-4 rounded-3xl p-6 text-rose-50">
          <h3 className="text-lg font-semibold text-rose-50">Coach alerts</h3>
          <ul className="space-y-3">
            {alerts.map((alert) => (
              <RedSurface key={alert.title} as="li" tone="glass" className="rounded-2xl p-4">
                <p className="text-sm font-semibold text-rose-50">{alert.title}</p>
                <p className="mt-1 text-sm text-rose-100/80">{alert.detail}</p>
              </RedSurface>
            ))}
          </ul>
        </RedSurface>
